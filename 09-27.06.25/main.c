...
int main() {
  ...
  do {
    ...
    calcMatrix(AnzZeilen1, AnzSpalten1, AnzSpalten2, Matrix1, Matrix2,
               MatrixErg);
    ...
    POSITION((AnzZeilen1 + AnzZeilen2) / 4 + 1,
             (AnzSpalten1 + AnzSpalten2) * 5 + 14);
    printf("=");
    printMatrix(1, (AnzSpalten1 + AnzSpalten2) * 5 + 17, AnzZeilenErg,
                AnzSpaltenErg, MatrixErg);
  } while (
      askAgain(2 + ((AnzZeilen1 > AnzZeilen2) ? AnzZeilen1 : AnzZeilen2), 1));
  CLEAR;
  HOME;
